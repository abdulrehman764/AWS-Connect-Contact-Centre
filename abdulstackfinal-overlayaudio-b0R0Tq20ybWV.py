...
import boto3
import os
# contactDetailsTableName	abdulstackcontactDetails
...
def handle_dynamo(event, contact_Id):
    ...
    # Extract callTimestamp and candidateNumber
    callTimestamp = response['Item']['callTimestamp']
    candidateNumber = response['Item']['candidateNumber']
    
    # Print the extracted values
    print("Call Timestamp:", callTimestamp)
    ...
    if status == "":
        ...
        status = merge_audio(event,candidateNumber,callTimestamp)
...
def merge_audio(event, candidateNumber, callTimestamp):
    print(event)
    ...
    try:
        s3r.Object(
        event['target']['Bucket'],
        f"{candidateNumber}_{callTimestamp}.wav"
